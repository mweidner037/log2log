...
import { defineJsonModel } from "../../src/models/json-model";
...
const model = defineJsonModel<Doc>();
...
describe("json-model proxy", () => {
  describe("reads like a plain object", () => {
    ...
    it("hides the MutableValue methods from JSON/keys/spread", () => {
      ...
      const spread = { ...m };
      assert.notProperty(spread, "_finish");
      assert.notProperty(spread, "_toImmutable");
    });
  ...
  describe("object mutations", () => {
    it("records a replace for an existing property", () => {
      ...
      assert.deepEqual(m._finish().updates, [
    ...
    it("records a remove for a deleted property", () => {
      ...
      assert.deepEqual(m2._finish().updates, [
  ...
  describe("array mutations", () => {
    ...
    it("keeps paths correct after a reordering", () => {
      ...
      second.name = "moved";
      const updates = m._finish().updates;
  ...
  describe("snapshots", () => {
    it("_toImmutable is isolated from later mutations", () => {
      const m = model.toMutable(newDoc());
      m.title = "v1";
      const snapshot = m._toImmutable();
      m.title = "v2";
      m.tags.push("c");
    ...
    it("_finish returns the final value plus all updates", () => {
      const m = model.toMutable(newDoc());
      m.title = "Final";
      m.tags.push("c");
